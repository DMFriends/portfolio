...
public class Palace
{
    ...
    public static void makeMove(Player p)
    {
        ...
        while(true)
        {
            ...
            System.out.print("\nPlayer " + p.getPlayerID() + ", what is your move? Separate multiple cards with a ', '. Type 'switch' to switch out a card from your hand with a 'banked' card. ");
            String playerMove = input.nextLine();
            
            if(!playerMove.equals("switch") && contains(",", playerMove))
            {
            ...
            else if((cardNotInHand(p, playerMove) || playerMove.equals("") || playerMove == null) && !playerMove.equals("switch"))
            {
                System.out.println("You do not have that card or you entered an invalid card. Please try again.\n");
                continue;
            }
            else if(!playerMove.equals("switch") && p.getHand().find(playerMove).isBanked())
            {
                System.out.println("That card is banked. You cannot play it. Please try again.\n");
                continue;
            }
            else if(playerMove.equals("switch"))
            {
                System.out.print("Player " + p.getPlayerID() + ", what card(s) in your hand do you want to switch? Separate multiple cards with a ', '. ");
                String cardInHand = input.nextLine();

                System.out.print("Player " + p.getPlayerID() + ", what card(s) do you want to switch to? Separate multiple cards with a ', '. ");
                String cardToSwitch = input.nextLine();

                if(validSwitchMove(p, cardInHand, cardToSwitch))
                {
                    break;
                }
                else
    ...
    public static boolean validSwitchMove(Player p, String cardInHand, String cardToSwitch)
    {
        boolean valid = false;

        if(contains(",", cardInHand) && contains(",", cardToSwitch))
        {
            ArrayList<Card> cardsInMove_Hand = createArrList(p, cardInHand);

            ArrayList<Card> cardsInMove_Switch = createArrList(p, cardToSwitch);

            for(Card card : cardsInMove_Hand)
            {
                if(card.isBanked() || (cardsInMove_Hand.size() != cardsInMove_Switch.size()))
                {
                    valid = false;
                    break;
                }
                ...
                {
                    p.getHand().find(card.toString()).setBanked(true);
                    valid = true;
                }
            }

            for(Card card : cardsInMove_Switch)
            {
                if(!card.isBanked() || (cardsInMove_Hand.size() != cardsInMove_Switch.size()))
                {
                    valid = false;
                ...
                {
                    p.getHand().find(card.toString()).setBanked(false);
                    valid = true;
                }
            }
            ...
            if(!valid)
            {
                System.out.println("Invalid input. Make sure the card(s) you want to switch are NOT ALREADY BANKED and the card(s) you want to switch to are BANKED. Please try again.");
            }
        }
        else
        {
            if(p.getHand().find(cardInHand) == null || p.getHand().find(cardToSwitch) == null)
            {
                System.out.println("You entered an invalid card. Please try again.");
                return false;
            }

            if(p.getHand().find(cardInHand).isBanked())
            {
                System.out.println(cardInHand + " is already banked. Please try again.\n");
                valid = false;
            }
            else
            {
                p.getHand().find(cardInHand).setBanked(true);
                valid = true;
            }

            if(!p.getHand().find(cardToSwitch).isBanked())
            {
                System.out.println(cardToSwitch + " is not banked. Please try again.\n");
                valid = false;
            }
            else
            {
                p.getHand().find(cardToSwitch).setBanked(false);
                valid = true;
            }
        }
    ...
    public static ArrayList<Card> createArrList(Player p, String card)
    {
        ...
        {
            temp[i] = temp[i].trim();
            arrList.add(p.getHand().find(temp[i]));
        }
